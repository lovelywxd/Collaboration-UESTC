...
def get_all_user(request):
    res = HttpResponse(serializers.serialize("json", User.objects.all())[1:-1])
    # return HttpResponse(User.objects.all().values())
    # print(User.objects.all().values())
    # test = {'a': 'b', 'c': 'd'}
    # res1 = JsonResponse(test)
    user = User.objects.all()
    print(user.values())
    l = []
    for u in user:
         l.append(json.dumps(u.to_dict()))
    # res2 = HttpResponse(json.dumps(user.to_dict()))
    user = User.objects.filter(username='1212')
    if len(user) <= 0:
        print("empty")
    res3 = HttpResponse(','.join(l))
    return res3
...
def promotion_list(request):
    ...
    return HttpResponse(",".join(promotionlist))
...
def add_favourite(request):
    ...
    favourite = UserFavourite.objects.filter(userName=request.user.name, bookName=bookname)
    if len(favourite) > 0:
