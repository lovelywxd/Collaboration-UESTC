...
//    NSString *url = @"http://192.168.1.100:8000";
    ...
    [appdele.manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject)
     {
         ...
         for (id promotion in responseObject) {
             NSDictionary *InfoDic = [promotion objectForKey:@"fields"];
             Activity *activity = [[Activity alloc] init:[promotion objectForKey:@"promotionID"]  withName:[InfoDic objectForKey:@"promotionName"] activiType:GroupBuy  urlString:[InfoDic objectForKey:@"promotionLink"]];
             [self.promotionList addObject:activity];
         }
         [self classifyActivity];
         
          NSString *promotionKey = @"4081/";
//         NSString* promotionKey = keys[0];
             ...
             [appdele.manager GET:url2 parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject)
              {
                  NSLog(@"promotion detail");
                  NSLog(@"djksjf");
                  for (id promotion in responseObject) {
                      NSLog(@"djksjf");
                  }
                  
//                  NSLog(@"promotion detail ,item amout:%ld",responseObject.count);
              }
